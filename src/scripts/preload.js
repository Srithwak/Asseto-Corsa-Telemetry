...
const readStatic = () => {
   ...
   return {
      ...
      playerFName: Array.from({ length: 33 }, () => readChar(reader)).join(''),
      playerLName: Array.from({ length: 33 }, () => readChar(reader)).join(''),
      playerNickname: Array.from({ length: 34 }, () => readChar(reader)).join(''),
      ...
      trackConfiguration: Array.from({ length: 34 }, () => readChar(reader)).join(''),
...
setInterval(() => {
   ...
   // console.log(combined.clutch);
   populateRenderer(combined);
   if (recording) raceData += `\n${Object.values(combined).join(',')}`;
}, interval);
